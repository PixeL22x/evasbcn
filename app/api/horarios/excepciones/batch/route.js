...
export async function POST(request) {
  try {
    ...
    // Validar datos
    const validTurnos = new Set(['M', 'T', 'L', 'DELETE'])
    ...
    for (const [trabajadorId, excepcionesTrabajador] of Object.entries(excepcionesPorTrabajador)) {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        // Esto borrará tanto las que vamos a sobrescribir como las que vamos a borrar (DELETE)
        ...
        // Preparar datos para createMany - FILTRAR 'DELETE'
        const dataToCreate = excepcionesTrabajador
          .filter(exc => exc.turno !== 'DELETE') // Skip DELETE items
          .map(excepcion => {
            // Crear fecha directamente en UTC para evitar problemas de zona horaria
